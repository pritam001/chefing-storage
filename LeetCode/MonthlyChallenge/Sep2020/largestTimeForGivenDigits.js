...
const largestTimeFromDigits = function(A) {
    let perms = permsFromArray(A);
    let filtered_perms = perms.filter(x => {
        let hour = parseInt(x.slice(0, 2), 10);
        let min = parseInt(x.slice(2, 4), 10);
        if(hour > 23) {
            return false;
        }
        if(min > 59) {
    ...
    filtered_perms.sort();
    let max_number = filtered_perms[filtered_perms.length - 1];
